  ...
  <v-content>
    <v-container fluid grid-list-md>
      <v-slide-y-transition mode="out-in">
        <v-layout v-if="likes" row wrap>
          <h1>My likes</h1>
          <v-flex v-for="like in likes" :key="like.artist + ' - ' + like.album" xs12>
            <v-card>
                ...
                <v-layout row align-content-left>
                  <v-flex xs2>
                    <v-card-media
                      ...
                      contain
                    >
                      <router-link class="cover-link" :to="createAlbumLink(like)"></router-link>
                    </v-card-media>
                  </v-flex>
                  <v-flex xs9>
                    <div>
                      <router-link class="album-link headline white--text" :to="createAlbumLink(like)">{{like.album}}</router-link><br>
                      <router-link class="album-link grey--text" :to="createArtistLink(like)">{{like.artist}}</router-link><br>
                    </div>
                  </v-flex>
                  <v-flex xs1>
                    <v-card-actions>
                      <v-btn icon @click="removeLike(like)">
                        <v-icon>close</v-icon>
                      </v-btn>
                    </v-card-actions>
                  </v-flex>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      removeLike(album) {
        this.$store.dispatch('removeLike', album);
      }
    },
    created() {
